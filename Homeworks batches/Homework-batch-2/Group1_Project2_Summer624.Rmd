...
# Add legend
legend("topleft", legend = "Predicted vs. Actual", col = c("blue", "red"),
       pch = c(19, NA), lty = c(NA, 1), cex = 0.8)
```

# Summary and Conclusion

In this project, we explored a dataset from a beverage manufacturing company with the goal of predicting pH values based on various production parameters. Our analysis involved data loading, exploration, cleaning, and model building using machine learning techniques.

- Key Findings:

Data Exploration: We found that the average pH of the beverages was 8.54, indicating a slightly alkaline nature typical for bottled water. The dataset contained 2571 observations with 33 variables, including numeric and categorical predictors such as brand codes and production metrics.

Data Cleaning: We handled missing data using MICE and removed outliers to improve model accuracy. Feature engineering involved creating dummy variables for categorical predictors like brand codes, enhancing the interpretability of the models.

- Model Building: We built three models to predict pH:

Linear Regression: Provided an R-squared of 0.41, suggesting a moderate fit to the data.
XGBoost: Achieved an R-squared of 0.68, demonstrating superior predictive performance due to its ability to capture complex relationships.
Support Vector Regression (SVR): Obtained an R-squared of 0.57, indicating good performance in capturing non-linear patterns.
Feature Importance: According to the XGBoost model, key predictors of pH included manufacturing flow rate (Mnf.Flow), oxygen filler content, brand codes, and various pressure metrics.

Prediction and Validation: Comparing actual vs. predicted pH values using XGBoost showed a strong linear relationship, confirming the model's effectiveness in predicting pH accurately.

- Recommendations:

Process Improvement: Focus on variables identified as important predictors to optimize pH control during production.
Data Quality: Continuously monitor and improve data collection to reduce missing values and ensure dataset completeness.

- Future Work:

Advanced Modeling: Explore other ensemble methods and neural networks for potential further improvements in predictive accuracy.
Real-time Monitoring: Implement real-time monitoring of production metrics to adjust processes dynamically based on predictive models.

This project underscores the importance of data-driven decision-making in maintaining product quality and process efficiency in beverage manufacturing.
